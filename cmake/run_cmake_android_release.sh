...
EDNA_BUILD_RELEASE_PATH=edna_build_android_x86_release
EDNA_INSTALL_RELEASE_PATH=edna_install_android_x86_release
...
if [ $# -ne 0 ]; then
    while getopts ":m:i:s:" opt; do
	  case $opt in
       ...
       ;;
	    ?)
	    echo "unknow param eg:[-m syallmode] or [-i allnavi] or [-i server] or [-i client] or [-s strip]"
	    exit 1;;
	  esac
     done
fi
...
if [[ ${_UseType} = "1" ]]; then
    ...
    EDNA_PLATFORM=_android_x86_release
   ...
   cp edna/funit/positioning/DR/SERVICE/OSAdaptor/libs/Android/armeabi-v7a/Release/libOSAdaptor.a edna/funit/positioning/DR/SERVICE/OSAdaptor/libs/
   cp edna/funit/positioning/DR/SERVICE/ConfigureCache/libs/Android/armeabi-v7a/Release/libConfigureCache.a edna/funit/positioning/DR/SERVICE/ConfigureCache/libs/
   cp edna/funit/positioning/DR/VehiclePosition/libs/Android/armeabi-v7a/Release/libDeadReckoning.a edna/funit/positioning/DR/VehiclePosition/libs/
   cp edna/funit/positioning/DR/VehiclePosition/libs/Android/armeabi-v7a/Release/libSensors.a edna/funit/positioning/DR/VehiclePosition/libs/

   cd ${EDNA_BUILD_RELEASE_PATH}
   cmake -DTARGET_ANDROID=ON -DTARGET_BUILDMODE=${TARGET_BUILDMODE} -DTARGET_SYALLMODE=${TARGET_SYALLMODE} -DANDROID_SDK=$ANDROID_SDK -DANDROID_SDK_API_LEVEL=29 -DANDROID_ABI=armeabi-v7a -DANDROID_JAR=$ANDROID_SDK/platforms/android-29/android.jar -DCMAKE_TOOLCHAIN_FILE=$ANDROID_NDK/build/cmake/android.toolchain.cmake -DWITH_PLUGINS=OFF -DWITH_DOCS=OFF -DWITH_TESTS=OFF -DDUMMY_GIT_VERSION=OFF -DNABASE_EGL_BACKEND=Android -DCMAKE_INSTALL_PREFIX=../${EDNA_INSTALL_RELEASE_PATH} -DUSE_QT=OFF -DCMAKE_BUILD_TYPE=Release -DDISABLE_WINDOW_HANDLING=OFF -DCMAKE_ANDROID_STANDALONE_TOOLCHAIN=$ANDROID_NDK/toolchains/arm-linux-androideabi-4.9/prebuilt/linux-x86_64/arm-linux-androideabi ../edna

elif [[ ${_UseType} = "3" ]]; then

    EDNA_PLATFORM=_android_arm64-v8a_release
    EDNA_BUILD_RELEASE_PATH=${EDNA_BUILD_FIRST_NAME}${EDNA_PLATFORM}${EDNA_LAST_NAME}
    EDNA_INSTALL_RELEASE_PATH=${EDNA_INSTALL_FIRST_NAME}${EDNA_PLATFORM}${EDNA_LAST_NAME}
    echo "EDNA_BUILD_RELEASE_PATH = ${EDNA_BUILD_RELEASE_PATH}"
    echo "EDNA_INSTALL_RELEASE_PATH = ${EDNA_INSTALL_RELEASE_PATH}"
   ...
   cp edna/funit/positioning/DR/SERVICE/OSAdaptor/libs/Android/arm64-v8a/Release/libOSAdaptor.a edna/funit/positioning/DR/SERVICE/OSAdaptor/libs/
   cp edna/funit/positioning/DR/SERVICE/ConfigureCache/libs/Android/arm64-v8a/Release/libConfigureCache.a edna/funit/positioning/DR/SERVICE/ConfigureCache/libs/
   cp edna/funit/positioning/DR/VehiclePosition/libs/Android/arm64-v8a/Release/libDeadReckoning.a edna/funit/positioning/DR/VehiclePosition/libs/
   cp edna/funit/positioning/DR/VehiclePosition/libs/Android/arm64-v8a/Release/libSensors.a edna/funit/positioning/DR/VehiclePosition/libs/

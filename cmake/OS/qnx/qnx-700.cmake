...
#         -G "Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=<path-to-src>/cmake/OS/qnx/qnx-660.cmake <path-to-src>
#   make
#
...
message(STATUS "qnx-700-cmake start")
...
set   (QNX_TARGET_ARCHITECTURE "aarch64le" CACHE STRING "QNX target architecture (e.g. 'x86', 'x86_64', 'aarch64le', 'armle-v7', ...)")
...
if ( QNX_TARGET_ARCHITECTURE STREQUAL "aarch64le")
   set( CMAKE_SYSTEM_PROCESSOR aarch64)
elseif ( QNX_TARGET_ARCHITECTURE STREQUAL "armle-v7")
   set( CMAKE_SYSTEM_PROCESSOR armv7)
elseif( QNX_TARGET_ARCHITECTURE STREQUAL "x86")
   set( CMAKE_SYSTEM_PROCESSOR x86)
elseif( QNX_TARGET_ARCHITECTURE STREQUAL "x86_64")
...
if(QNX_USE_LIBCPP)
   ...
   ########################################################
   # Dinkumware lib in QNX 7.0.0 is compiled in C++11 mode,
   # we need to stay compatible to avoid crashes
   SET(NA_GCC_CXX_STD "-std=c++11")
   ########################################################

   set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -D_EXCEPTION -D_C99 -nostdinc -nostdinc++")
   set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${CMAKE_SOURCE_DIR}/cmake/OS/qnx/700/include")
   set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include")
   set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/v1")
   # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/cpp")
   # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0")

   # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/v1")
   # set( QNX_CPP_STDLIB "-nodefaultlibs -lcpp -lnbutil -lm -lcxa -lc -lsocket")
   # set( QNX_CPP_STDLIB "-nodefaultlibs -lstdc++ -lc++ -lnbutil -lm -lc -lsocket")
   set( QNX_CPP_STDLIB "-nodefaultlibs -lc++ -lnbutil -lm -lc -lsocket")

   if(CMAKE_SYSTEM_PROCESSOR STREQUAL "aarch64")
      # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0/aarch64-unknown-nto-qnx7.0.0")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_HOST}/usr/lib/gcc/aarch64-unknown-nto-qnx7.0.0/5.4.0/include")
   elseif(CMAKE_SYSTEM_PROCESSOR STREQUAL "armv7")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0/arm-unknown-nto-qnx7.0.0eabi0")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_HOST}/usr/lib/gcc/arm-unknown-nto-qnx7.0.0eabi0/5.4.0/include")
   elseif(CMAKE_SYSTEM_PROCESSOR STREQUAL "x86")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -fno-stack-protector")
      set( QNX_C_FLAGS "${QNX_C_FLAGS} -fno-stack-protector")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0/x86_64-pc-nto-qnx7.0.0")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_HOST}/usr/lib/gcc/x86_64-pc-nto-qnx7.0.0/5.4.0/include")
   elseif(CMAKE_SYSTEM_PROCESSOR STREQUAL "x86_64")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -fno-stack-protector")
      set( QNX_C_FLAGS "${QNX_C_FLAGS} -fno-stack-protector")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0/i586-pc-nto-qnx7.0.0")
      set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_HOST}/usr/lib/gcc/i586-pc-nto-qnx7.0.0/5.4.0/include")
   endif()

   # include later
   # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -isystem ${QNX_TARGET}/usr/include/c++/5.4.0/tr1")

   # set( QNX_CPP_STDLIB "${QNX_CPP_STDLIB} -Wl,-L${QNX_TARGET}/${CMAKE_CXX_LIBRARY_ARCHITECTURE}/lib/gcc/5.4.0")
   ...
   # Workaround Boost compilation problem
   set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -DBOOST_CHRONO_LIB_CONSTEXPR=const")
   # set( QNX_CXX_FLAGS "${QNX_CXX_FLAGS} -fpermissive")
endif()
...
set(CMAKE_EXE_LINKER_FLAGS    "--sysroot=${QNX_TARGET} ${QNX_CPP_STDLIB}"                CACHE STRING "Linkerflags for executables")
set(CMAKE_SHARED_LINKER_FLAGS "--sysroot=${QNX_TARGET} ${QNX_CPP_STDLIB} -Wl,-Bsymbolic" CACHE STRING "Linkerflags for shared libs")
set(CMAKE_MODULE_LINKER_FLAGS "--sysroot=${QNX_TARGET} ${QNX_CPP_STDLIB} -Wl,-Bsymbolic" CACHE STRING "Linkerflags for modules"    )
...
message(STATUS "qnx-700-cmake end")
